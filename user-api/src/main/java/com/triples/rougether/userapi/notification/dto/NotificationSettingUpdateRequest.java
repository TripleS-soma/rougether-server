...
public record NotificationSettingUpdateRequest(
        @Schema(description = "전체 알림 마스터 스위치. false면 reminder·house·feed 값과 무관하게 모든 push가 중단됨. "
        ...
        Boolean house,
        @Schema(description = "피드 댓글 알림 수신 여부. 바꾸지 않으려면 생략", example = "true")
        Boolean feed
) {

    // 부분 전송이라 모든 필드가 optional — 전부 생략되면 바꿀 게 없으므로 400.
    @JsonIgnore
    @Schema(hidden = true)
    @AssertTrue(message = "변경할 항목을 최소 하나 지정해야 합니다.")
    public boolean isAnyPresent() {
        return all != null || reminder != null || house != null || feed != null;
    }
}
