...
process = subprocess.Popen([str(ROOT / "ai-service/.venv/bin/python"), "-m", "uvicorn",
    "rougether_ai.app:create_app", "--factory", "--host", "127.0.0.1", "--port", str(port), "--no-access-log"],
...
try:
    for _ in range(100):
        ...
        try:
            with urllib.request.urlopen(f"http://127.0.0.1:{port}/health/ready", timeout=1):
                break
        except OSError:
            time.sleep(0.1)
    else:
        raise RuntimeError("AI 서비스 기동 제한 시간 초과")
    env["AI_SERVICE_CONTRACT_URL"] = f"http://127.0.0.1:{port}"
