...
기본 seed는 사용자·지갑·개인 방·투두를 생성한다. 과거 완료 이력은 `--history-per-user` 옵션으로 추가한다. 온보딩 목표·캐릭터와 루틴은 포함하지 않으므로 활동이 많은 실제 사용자의 전체 조회 비용을 대표하지 않는다. 기본 회차와 과거 이력을 채운 회차를 구분한다.
