-- 테마 공통 벽지·바닥·가구 3종을 비활성 상태로 준비하는 1차 이행임.
-- 풀 검증 및 새 서버 전체 배포 성공 후 별도 운영 활성화로 공개하며, 기존 머신은 수정하지 않음.
...
CREATE TEMPORARY TABLE gacha_category_entries_v66 AS
...
SELECT category.code, category.name, 'COIN', 25, 1,
       NULL, NULL, FALSE, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL
FROM (
...
-- 사전 등록된 정식 코드도 동일한 비용·기간·테마 계약과 비활성 준비 상태로 맞춤.
UPDATE gacha
SET name = CASE code
...
SELECT target.id, 'ITEM', source.item_id, NULL, NULL, NULL,
       ...
       1, TRUE
FROM gacha_category_entries_v66 source
...
-- 기존 앱의 목록과 캐시된 gachaId를 보존함. 구머신 종료는 별도 이행으로 결정함.
-- 운영 활성화 이후 새 앱이 catalog=category로 세 정식 머신을 조회함.
DROP TABLE gacha_category_entries_v66;
