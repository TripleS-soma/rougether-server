...
스펙을 읽어 `$ARGUMENTS` 범위의 **구현 계획**을 세우고 `docs/work/`에 기록한다. 이 커맨드는 코드를 구현하지 않는다 — 계획만 확정한다. 구현은 plan이 확정된 뒤 `/run-plan`이 자율로 수행한다.
...
- 이 커맨드는 코드를 구현하지 않으므로 `./gradlew` 실행·커밋·PR을 하지 않는다.
