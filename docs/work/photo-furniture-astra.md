...
검수는 동일한 물체 높이로 기존 에셋과 나란히 놓았을 때 한 가구 세트처럼 보이는지 비교합니다. 형태가 정상이고 원본을 닮았다는 이유만으로 통과시키지 않으며, 6개 기준 모두 충족해야 ACCEPT를 선택하도록 요청합니다. 수정 사유에는 참고 에셋 대비 구체적인 차이와 보존할 식별 특징을 적도록 합니다. 이 기준은 모델에 주는 지침이며 실제 스타일 일치를 결정적으로 보장하지 않습니다.
...
위 실사진 검증 이후 사용자가 실물에 가까운 비율·질감과 강한 색감 때문에 루게더 스타일과 차이가 있다고 평가했습니다. 이에 생성 전 스타일 지침과 검수 기준을 위 6개 항목으로 강화했습니다. 강화한 지침으로 새 유료 이미지를 생성한 검증은 아직 하지 않았으며, 앞선 ACCEPT를 새 스타일 기준의 통과 증거로 사용하지 않습니다.
