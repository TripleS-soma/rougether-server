...
이전 격리 실험에서 실제 Lambda 및 모의 AI HTTP 동시 50개, 100개 작업의 정산 정합성을 확인했다. VPC 내부 API의 부하 중 p95는 두 회차에서 약 11.8ms/10.5ms였고 일반 API 5,112건이 모두 성공했다. 실제 OpenAI 전체 흐름은 동시 2/4/8, 총 14건까지 확인했다. 실제 OpenAI 동시 50건의 처리량을 측정했다는 뜻은 아니다.

서비스에 옮긴 코드에서 전체 Gradle 테스트 2,080개 중 2,073개 통과, opt-in 등 7개 제외, 실패·오류 0개를 확인했다. API bootJar와 AI/control ZIP을 함께 빌드했다. 실서비스 배포 후 실제 사진 2건으로 큐부터 정산·보관함까지 확인한다. 세부 원자료와 최종 상태는 원 저장소 output/releases/furniture-service-20260911에 기록한다.
