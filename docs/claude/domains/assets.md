...
- 뽑기 선물상자: `GET /api/v1/gacha?catalog=category`는 `category=WALLPAPER/FLOOR/FURNITURE`인 테마 무관 세 종류다. `catalog` 생략 시 기존 테마·캐릭터 목록을 유지한다. `giftBoxAssetKey`는 카테고리별 투명 PNG key이며 다른 이미지와 동일하게 CDN base URL과 조합한다. 이전 머신 직접 상세 조회의 테마별 key와 캐릭터 기본 선물상자도 유지한다. 불투명 핑크 배경이 남은 `items/698ebc78-8273-4bf7-85d4-a7ea81c7c4d0.png`는 응답에 사용하지 않는다.
