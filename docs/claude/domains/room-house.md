...
- 온보딩 목표·캐릭터 선택이 처음 모두 갖춰지는 요청에서 기본 공동집을 같은 트랜잭션으로 생성합니다. 목표→캐릭터와 캐릭터→목표 순서를 모두 지원하며, 두 쓰기 경로가 사용자 행 락을 공유해 동시 요청·재시도에도 한 번만 생성합니다.
- 기본 집은 이름 `나의 집`, 정원 4명, 게시 승인 커버 manifest의 첫 항목으로 시작합니다. 집 목표는 대표 목표를 우선하고 나머지는 목표 마스터 정렬순으로 채워 최대 3개를 연결합니다. 이후 일반 집 설정 API로 변경할 수 있습니다.
