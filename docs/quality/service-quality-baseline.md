...
| API 지연시간, 오류율, 처리량 | 값 없음 | 미측정 | `health`, `info` 외 운영 metric 미수집 |
...
| `Q-API-02` | HTTP 5xx 응답 수 / 전체 API 응답 수, 일 단위 | 1.0% 미만 | 미측정 |
| `Q-API-03` | upload/AI 제외 일반 API server latency | p95 500ms 이하, p99 1초 이하 | 미측정 |
...
3. **QA 시나리오 부하 테스트**: 로그인, 루틴 완료, 투두 완료, 집 가입, 미션 기여/claim을 실제 API 순서로 실행하고 동시 사용자 수, throughput, p95/p99, 오류율을 기록한다.
